...
4. (Optional) override or add environment variables in `.env` (or copy from `.env.example`):
   ...
   - `OPENAI_TEMPERATURE` (optional, default `0.1`, clamped to `[0.0, 2.0]`; automatically omitted for GPT-5 family models)
...
- `GET /api/analysis/{symbol}` -> detail for a symbol (`company_name`, `scenarios`, `key_variables`, assumptions + summary fields).
...
- `PUT /api/configuration/prompts` with body `{"templates":{"analysis_prompt_company_name":"...","analysis_prompt_business_model":"...","analysis_prompt_key_variables":"...","analysis_prompt_scenarios":"..."}}` -> saves custom prompt templates (each validated for required variables).
...
- `analysis_symbols(id INTEGER PK, symbol UNIQUE, company_name, current_price, expected_price, upside, overall_confidence, assumptions_text, business_model_text, business_summary_text, raw_ai_response, timestamps)`
