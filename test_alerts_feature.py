...
class AlertsUiStructureTests(unittest.TestCase):
    ...
    def test_earnings_review_has_separate_list_and_detail_views(self):
        ...
        self.assertIn("if (releaseTime < yesterday.getTime()) return 'past';", js)
        self.assertIn("if (releaseTime === yesterday.getTime()) return 'yesterday';", js)
        self.assertIn("if (releaseTime === tomorrow.getTime()) return 'tomorrow';", js)
        self.assertIn("return 'future';", js)
